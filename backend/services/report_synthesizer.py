...
class ReportSynthesizer:
    ...
    def generate_summary_judgement(
        ...
        title = (
            _value(metadata, "document_reviewed", "the reviewed document")
            or "the reviewed document"
        )
    ...
    def generate_bottom_line(self, blockers: Iterable[Any]) -> str:
        ...
        if first is None:
            return (
                "Tidak ada temuan penghalang; tutup temuan yang tersisa pada tinjauan terkendali berikutnya."
                if self.s.bottom_line != "BOTTOM LINE"
                else "No blocking finding was identified; close the remaining findings in the next controlled review."
            )
        ...
        return (
            f"Selesaikan {title} sebelum penerbitan ulang: {fix}."
            if self.s.bottom_line != "BOTTOM LINE"
            else f"Resolve {title} before reissue: {fix}."
        )
    ...
    def generate_recommendations(self, findings: Iterable[Any], limit: int = 5) -> list[str]:
        ...
        for item in findings:
            ...
            default = {
                "UNCONTROLLED_PAGE": "Terapkan template dokumen terkendali pada seluruh halaman terdampak.",
                "CROSS_PAGE_BREAK": "Perbaiki pagination agar kalimat dan paragraf tidak terputus lintas halaman.",
                "REFERENCE_DRIFT": "Perbarui referensi halaman dan verifikasi ulang daftar isi.",
            }.get(
                issue_type,
                "Perbaiki temuan, verifikasi bukti sumber, dan terbitkan revisi terkendali.",
            )
            action = str(_value(item, "suggestion", "") or default).strip()
        ...
        for item in findings:
            ...
            suggestion = str(
                _value(
                    item,
                    "suggestion",
                    "Correct the identified issue and verify the revised section.",
                )
            )
            page = str(
            ...
            rows.append(
                {
                    "location": f"Page {page}" if page not in {"", "None"} else "Not located",
                    "type": issue_type,
                    "excerpt": excerpt,
                    "action": suggestion,
                }
    ...
    def generate_demonstration_rewrite(
        ...
        return {
            "section_title": "Conclusions" if self.s.bottom_line == "BOTTOM LINE" else "Kesimpulan",
    ...
    def synthesize_baseline_measures(
        ...
        return [
            {"measure": n, "result": "PASS" if ok else "FAIL", "reason": reason}
