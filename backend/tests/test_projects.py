...
async def test_finish_rejects_processing_document() -> None:
    lead = _user(UserRole.LEAD_ENGINEER)
    document = Document(
        id=uuid4(), original_filename="queued.pdf", safe_filename="queued", media_type="application/pdf",
        size_bytes=1, sha256="a" * 64, storage_uri="local://queued.pdf",
        status=DocumentStatus.PROCESSING, progress_pct=42,
    )
...
async def test_finished_project_rejects_assignment() -> None:
    ...
    with pytest.raises(HTTPException) as error:
        await assign_project(project.id, ProjectAssignment(assigned_to_id=engineer.id), session, lead)
