...
def test_extract_pdf_valid(sample_pdf_path):
    ...
    # fitz will raise a DLL load failure, and our extractor will catch it and return warnings.
    if artifact.warnings and any("DLL load failed" in w for w in artifact.warnings):
        pytest.skip("Environment is missing PyMuPDF DLL dependencies. Skipping assertion of extracted content.")
...
def test_extract_invalid_file_type(tmp_path):
